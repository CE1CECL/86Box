...
recalculate_data(void *priv)
{
     ...
     * dev->vbe_regs[VBE_DISPI_INDEX_VIRT_HEIGHT] < dev->vbe_regs[VBE_DISPI_INDEX_YRES].
            ...
            switch (dev->vbe_index) {
                case VBE_DISPI_INDEX_ID:
                    if (val == VBE_DISPI_ID0 || val == VBE_DISPI_ID1 || val == VBE_DISPI_ID2 || val == VBE_DISPI_ID3 || val == VBE_DISPI_ID4 || val == VBE_DISPI_ID5 ||
                        /* VBox extensions. */
                        val == VBE_DISPI_ID_VBOX_VIDEO || val == VBE_DISPI_ID_ANYX ||
                ...
                case VBE_DISPI_INDEX_ENABLE:
                    {
                        if ((val & VBE_DISPI_ENABLED) && !(dev->vbe_regs[VBE_DISPI_INDEX_ENABLE] & VBE_DISPI_ENABLED)) {
                            ...
                            if (!cVirtWidth
                                || !dev->vbe_regs[VBE_DISPI_INDEX_YRES]
                                || cb > dev->vram_size) {
                                printf("VIRT WIDTH=%d YRES=%d cb=%d vram_size=%d\n",
                                       dev->vbe_regs[VBE_DISPI_INDEX_VIRT_WIDTH], dev->vbe_regs[VBE_DISPI_INDEX_YRES], cb, dev->vram_size);
                             ...
                             * Or may be these update function should take the dev->vbe_regs[dev->vbe_index]
...
bochs_vbe_init(const device_t *info)
{
    ...
    if (dev->id5_val == VBE_DISPI_ID4) {
        rom_init(&dev->bios_rom, "roms/video/VBoxVgaBiosBin/VBoxVgaBios386.rom",
                 0xc0000, 0x10000, 0xffff, 0x0000,
                 MEM_MAPPING_EXTERNAL);
    } else {
        rom_init(&dev->bios_rom, "roms/video/bochs-emu-VGABIOS/VGABIOS-lgpl-latest-debug.bin",
                 0xc0000, 0x10000, 0xffff, 0x0000,
                 MEM_MAPPING_EXTERNAL);
    }
...
{
    if ((rom_present("roms/video/bochs-emu-VGABIOS/VGABIOS-lgpl-latest-debug.bin")) && (rom_present("roms/video/VBoxVgaBiosBin/VBoxVgaBios386.rom"))) {
        return 1;
    } else {
        return 0;
    }
...
static device_config_t bochs_vbe_config[] = {
    { .name           = "revision",
     ...
     .selection      = {
          ...
          { .description = "" } },
     .bios = { { 0 } } },
    { .name = "memory", .description = "Memory size", .type = CONFIG_SELECTION, .default_string = NULL, .default_int = 512, .file_filter = NULL, .spinner = { 0 }, .selection = { { .description = "128 MB", .value = 128 }, { .description = "256 MB", .value = 256 }, { .description = "512 MB", .value = 512 }, { .description = "" } }, .bios = { { 0 } } },
